...
from utils.proposed_mothod import filter_selection, B_stream_to_sparse_masks, secret_model_extraction, secret_model_embedding, side_info_embedding, side_info_extraction, bn_running_extraction, bn_running_embedding
...
if __name__ == '__main__':
    ...
    logger.info('Extracting the running mean and var in the  tuned_sub_model')
    bn_running_binary = bn_running_extraction(tuned_sub_model)
    logger.info('Embedding the B_stream and bn_running info into the stego model.')
    stego_model = torch.load(model_save_path)
    stego_model = side_info_embedding(key, stego_model, B_stream, bn_running_binary)
